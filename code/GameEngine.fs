...
let rec playGame =
    fun gs ->
        ...
        printfn "Current room's objects: %A" objects
                ...
                *)
                ...
                if mc.stats.hp > 0 && e.stats.hp > 0 then
                    ...
                    if mc.stats.hp <= 0 then 
                        playGame ({ characters = gs.characters;
                                    rooms = gs.rooms;
                                    objects = gs.objects;
                                    turn = 0;
                                    mcL = gs.mcL;
                        ...
                        let currentRoom = 
                            ...
                            | None ->
                                ...
                                exit 1
                        let updatedObjects = List.removeAt (List.findIndex (fun s ->
                                match s with
                                | Character c ->
                                    if c = e then 
                                        true
                        ...
                        playGame ({ characters = gs.characters |> Map.remove e.name;
                                    rooms = gs.rooms |> Map.add currentRoom.name currentRoom;
                                    objects = gs.objects;
                                    turn = 0;
                                    mcL = gs.mcL;
                                    alive = gs.alive})
        ...
        *)
        if gs.alive = true then
            ...
            | "help" | "h" ->
                ...
                printfn "   'fight <character>' or 'attack character': initiates combat with specified character."
                playGame gs
            ...
            | "fight" | "attack" ->
                let nchar = 
                    match characters |> Map.remove mc.name |> Map.filter (fun n c -> (
            ...
            | _ when (containsObjects input) ->
                ...
                printfn "Possible Interactions: %A" interactions
